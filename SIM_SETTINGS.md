...
## Full UE Trajectory Overview

![All scenario UE trajectories and BS camera FoV](./docs/images/scenarios/all_scenario_trajectory_fov_map.png)

This figure shows the UE trajectory samples for `sc01` to `sc08` in the global
CARLA XY coordinate frame. It also overlays each BS location and approximate BS
camera FoV, making it easier to see how the scenarios differ in route coverage,
BS placement, and viewing direction. The figure was generated by
`wireless_dataset_validation/05_environment_trajectory.ipynb` and copied from
`wireless_dataset_validation/figures_mpl/10_all_scenario_trajectory_fov_map.png`
so it can be rendered on GitHub.
