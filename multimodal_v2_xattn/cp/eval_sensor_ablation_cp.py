"""멀티모달 채널 예측 run(`mm:chiron` · `mm2:chiron`) 의 **센서 기여 절제 평가** — 학습 없음, best.pt 재평가만 (2026-09-07, 신규 파일; 2026-09-20 mm2: 지원 — 게이트 진단 생략, attention_mass = 식(21) 모달별 가중치).
...
assert cfg["model"].startswith("mm:") or cfg["model"].startswith("mm2:"), f"mm:/mm2: 모델 전용 (config model={cfg['model']}); 채널 전용 run 은 eval_sanity.py"
...
kw.update(fuse_where=cfg.get("fuse_where", "input"), fuse_zero_init=not cfg.get("no_fuse_zero_init", False), fuse_no_channel_token=cfg.get("fuse_no_channel_token", False))   # mm2: 전용(2026-09-20); mm: 은 무시
...
IS_V2 = bool(len(model.fusion)) and not hasattr(model.fusion[0], "gate")   # mm2: 융합 블록은 게이트 없음(식 21 단일 MHA) → 게이트·norm-ratio 진단 생략, 어텐션(모달별)만
...
def run_eval(wins, conds, diag, tag):
    ...
    dg = dict(n=0, gate=None, mass=None, frame=None, ratio=None); probe = FusionProbe(model) if (diag and len(model.fusion) and not IS_V2) else None; diag_v2 = bool(diag and len(model.fusion) and IS_V2); selfcheck = None; t0 = time.time(); load_sec = 0.0
    for bi, b in enumerate(range(0, n, a.batch)):
        ...
        if probe is not None or diag_v2:   # real 입력 진단 forward(diag=True: 어텐션 가중치 저장; need_weights 경로라 real 지표와 분리)
            model(X, S, diag=True); gm = np.array(model.gate_means(), dtype=np.float64); mass = model.attention_mass()
            rat = probe.ratios() if probe is not None else [dict(upd_over_z=0.0, attn_over_z=0.0) for _ in model.fusion]   # mm2: 게이트 없음 → 0 자리표시
            fr = [None if IS_V2 else (blk.last_attn.mean(dim=(0, 1)).view(K, -1).sum(1).cpu().numpy() if blk.last_attn is not None else None) for blk in model.fusion]   # mm2: 프레임별 소프트맥스 합이 1 이라 frame mass 무의미
    ...
    if dg["n"]:
        ...
        per_tok = [{m: v / nt.get(m, 1) for m, v in ms.items()} for ms in mass]; share = [{m: v / max(1e-12, sum(ms.values())) for m, v in ms.items()} for ms in per_tok]
