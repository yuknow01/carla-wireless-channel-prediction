...
sys.path.insert(1, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))   # 폴더 루트: `models/chiron_channel.py` 사본 (multimodal_v2_xattn, 2026-09-20)
...
# --- 멀티모달 v2(mm2:) 전용 인자 (2026-09-20). [B] 식(20)(21) 융합. mm2: 가 아닌 모델에서는 어디에도 쓰이지 않는다 ---
ap.add_argument("--fuse_where", default="input", choices=["input", "output"], help="mm2: 융합 토큰 주입 위치. input = 패치 임베딩 직후·chiron 블록 앞([B] 위치), output = 백본 뒤·헤드 앞(v1 위치)")
ap.add_argument("--no_fuse_zero_init", action="store_true", help="mm2: 융합 out_proj 0 초기화를 끔([B] 원형)")
ap.add_argument("--fuse_no_channel_token", action="store_true", help="mm2: U 에서 채널 토큰 c_k 제외(센서끼리만 attend — 9/18 미팅 제안 A)")
args = ap.parse_args()
IS_MM2 = args.model.startswith("mm2:")   # v2 (cp_multimodal_v2.py, 2026-09-20)
IS_MM = args.model.startswith("mm:") or IS_MM2   # mm: 과 mm2: 모두 센서 로더(MMWindowSet)·forward(X, sensors) 경로
...
if IS_MM2: kw.update(fuse_where=args.fuse_where, fuse_zero_init=not args.no_fuse_zero_init, fuse_no_channel_token=args.fuse_no_channel_token)   # heads 8·LiDAR 토큰 1 은 코드 고정([B] 값)
